...
const englishText: Record<string, string> = {
  ...
  工号或邮箱: "Staff ID or Email",
  请输入工号或学校邮箱: "Enter your staff ID or university email",
  密码: "Password",
  ...
  "账号资料与登录角色不一致，请联系管理员。 requestId：请查看 /me 响应":
    "The account profile does not match the signed-in role. Contact an administrator. requestId: see the /me response.",
  ...
  教师内部备注: "Internal teacher note",
  仅教师可见: "Visible to teachers only",
  ...
  材料类别: "Material type",
  提交人: "Submitted by",
  查看权限: "Viewing permission",
  教师审核只读: "Teacher review: read only",
  ...
  只读模式会拦截所有写操作: "Read-only mode blocks all write actions",
  维护模式会立即阻止普通用户进入:
  ...
  "查看本次打卡的完整数据和运动凭证；此处调整也只保存在当前前端页面。":
    "View complete check-in data and activity evidence. Adjustments here are saved only on the current front-end page.",
  保存前端调整: "Save front-end adjustment",
  可计入时长: "Counted duration",
  "h（实际": "h (actual ",
  "· 置信度": "· confidence ",
  预览原件: "Preview original",
  "审核说明（前端备注）": "Review note (front-end)",
  保存成绩: "Save grade",
  ...
  "后端合同不提供单独撤销接口。生成新邀请码会在同一服务端事务中使旧邀请码失效，且不会影响已经建立的成员关系。":
    "The backend contract has no standalone revoke endpoint. Creating a new invitation invalidates the old one in the same server transaction without affecting existing enrollments.",
  生成新码并替换旧码: "Generate a new code and replace the old one",
  "查看本次打卡的完整数据和运动凭证；演示调整仅保存在当前页面。":
    "View the full check-in data and evidence; demo adjustments stay on this page only.",
  "查看服务端记录与真实运动凭证。有效/无效结论请在审核工作台提交。":
    "View the server record and genuine evidence. Submit valid/invalid decisions in the review workbench.",
  保存演示调整: "Save demo adjustment",
  "合同明确禁止教师覆盖计入时长。教师可以追加“有效/无效”审核记录，但不能在客户端改写服务端时长事实。":
    "The contract prohibits teacher duration overrides. Teachers may append valid/invalid reviews but cannot rewrite server duration facts from the client.",
  服务端成绩: "Server score",
  ...
  "发布请求逐条使用服务端版本控制。页面不会声称已发送合同未保证的通知。":
    "Each publish request uses server version control. The page does not claim notifications that the contract does not guarantee.",
  "该申请未附带可访问的服务端凭证。":
  ...
  "服务端合同当前未提供风险或置信度投影":
    "The server contract currently exposes no risk or confidence projection.",
  "服务端教师投影当前不返回位置或地图数据":
  ...
  "查看服务端学期状态；当前合同不提供手工创建、切换或归档操作。":
    "View server term status; the current contract does not provide manual creation, switching, or archiving.",
  "查看组织范围内的账号与角色资料；当前合同不提供账号恢复、解锁或删除操作。":
    "View organization-scoped accounts and roles; the current contract does not provide account recovery, unlocking, or deletion.",
  "查看组织范围内的用户反馈；当前合同不提供回复或状态变更操作。":
    "View organization-scoped user feedback; the current contract provides no reply or status mutation.",
  "维护服务端总学时成绩规则草稿，并执行双管理员审批流程。":
    "Maintain server total-hours score-rule drafts and complete the two-administrator approval flow.",
  "查看当前服务端系统模式；客户端不显示合同未开放的切换操作。":
    "View the current server system mode; the client hides switching actions not exposed by the contract.",
  "查看服务端已发布的中英文帮助内容；发布能力不在当前客户端合同中。":
    "View published Chinese and English server help; publication is outside the current client contract.",
  "追踪服务端关键操作；审计记录只读，不可修改或删除。":
    "Trace important server operations; audit records are read-only and cannot be changed or deleted.",
  "请输入 2–32 位学校组织代码（大写字母、数字、下划线或连字符）。":
    "Enter a 2–32 character organization code using uppercase letters, numbers, underscores, or hyphens.",
  "请输入账号已绑定的完整邮箱地址。":
  ...
  "输入学校组织代码、账号绑定邮箱和账号身份。后端会创建一次性恢复请求，并通过已配置的邮件服务发送验证码。":
    "Enter the organization code, account email, and account role. The server will create a one-time recovery request and send a verification code through the configured email service.",
  学校组织代码: "Organization code",
  "例如 BNBU": "For example, BNBU",
  账号身份: "Account role",
  管理员: "Administrator",
  ...
  "至少 12 位字符": "At least 12 characters",
  "正在验证…": "Verifying…",
  验证并重置密码: "Verify and reset password",
  "已登录状态下修改密码尚未纳入 Backend 合同；请退出后使用真实邮箱恢复流程重置密码。":
    "Changing a password while signed in is not yet part of the Backend contract. Sign out and use the live email recovery flow to reset it.",
  已忽略: "Ignored",
  入学年份: "Entry year",
  学院: "School",
  ...
  受合同约束的处理范围: "Contract-governed action scope",
  ...
  "字段名与后端合同一致；学号和姓名均为必填字段，最终校验结果以后端为准。":
    "Field names follow the Backend contract. Student number and name are required, and server validation is authoritative.",
  ...
  "本页只显示后端 StudentScore 投影；导出合同当前为默认拒绝，因此不提供本地拼接 CSV。":
    "This page displays only the server StudentScore projection. Export is default-denied by the current contract, so the client does not assemble a local CSV.",
 ...
 * backend enum such as `READ_ONLY` or `pending` directly in the UI.
 */
