// Contract binding check for the pinned Backend OpenAPI snapshot.
//
// The Backend publishes one immutable snapshot per contract version and states
...
if (invokedDirectly) {
  ...
  const lines = [
    `contract version : ${report.descriptor.contractVersion}`,
    `snapshot version : ${report.declaredVersion}`,
