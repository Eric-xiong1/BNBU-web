...
import { renderProfile, renderAccountDetails, renderSettings, profileActions } from "./screens/profile.js";
...
export const app = {
  ...
  state: {
    ...
    systemMode: params.get("sysmode") === "maintenance" ? "MAINTENANCE" : params.get("sysmode") === "readonly" ? "READ_ONLY" : "NORMAL",
    systemModeStatus: {
      message: "",
      estimatedRecoveryTime: params.get("sysmode") === "maintenance" ? "2026-07-29 06:00" : null,
      plannedMaintenanceAt: params.get("sysmode") === "planned" ? "2026-08-01 02:00" : null,
    },
  ...
  // ── Derived helpers ─────────────────────────────────────────
  isWriteAllowed() { return this.state.systemMode !== "READ_ONLY"; },
  hasActiveEnrollment() {
  ...
  applyOverlay(workspace) {
    ...
    workspace.notices = workspace.notices.map((n) => (o.readNoticeIds.includes(n.id) ? { ...n, isUnread: false } : n));
    ...
    if (o.exemptions.length) workspace.exemptions = [...o.exemptions, ...workspace.exemptions];
    return workspace;
  },

  markNoticeRead(id) {
    if (!this.overlay.readNoticeIds.includes(id)) this.overlay.readNoticeIds.push(id);
    this.saveOverlay();
    const notice = this.state.workspace.notices.find((n) => n.id === id);
    if (notice) notice.isUnread = false;
  },
  markAllNoticesRead() {
    for (const n of this.visibleNotices()) if (n.isUnread) this.markNoticeRead(n.id);
  },
  ...
  renderAuthContent() {
    const s = this.state;
    if (s.isRestoringSession || !s.privacyConsentChecked) return renderStartupSplash(this);
