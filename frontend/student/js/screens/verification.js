...
function derived(state) {
  ...
  const contactInvalid = state.contact.trim() !== "" && !emailValid;
  const codeValid = /^\d{4,10}$/.test(state.code);
  const codeInvalid = state.code !== "" && !codeValid;
  return {
    contactInvalid,
    codeInvalid,
      ...
      <div class="auth-column" style="padding-top:30px">
        ...
        <div class="vlogin-card">
          ...
          ${inputField({
            ...
            trailing: `<button class="text-btn pressable vfield-trailing" data-action="verification.sendCode" ${rules.canSend ? "" : "disabled"} style="min-height:48px;padding:0 8px">
          ...
          ${state.info || state.error ? `<div style="padding-top:18px" class="col gap8">${state.info ? statusBanner(state.info, false) : ""}${state.error ? statusBanner(state.error, true) : ""}</div>` : ""}
          <div style="height:24px"></div>
          <button class="vlogin-submit pressable" data-action="verification.submit" ${rules.canLogin ? "" : "disabled"}>
...
function syncControls(app) {
  ...
  if (send) send.disabled = !rules.canSend;
  const submit = app._viewport?.querySelector('[data-action="verification.submit"]');
  if (submit) submit.disabled = !rules.canLogin;
}
...
export const verificationActions = {
  ...
  "verification.sendCode": async (app) => {
    const state = verificationState(app);
    if (!derived(state).canSend) return;
    state.sending = true;
    state.error = null;
    state.info = null;
    app.render();
    try {
      ...
      state.error = diagnosticError(error);
    } finally {
      state.sending = false;
      app.render();
  ...
  "verification.submit": async (app) => {
    const state = verificationState(app);
    if (!derived(state).canLogin) return;
