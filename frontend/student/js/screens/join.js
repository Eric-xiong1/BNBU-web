...
function scanMessage(text) {
  return `<div class="scan-message" role="status">
    ...
    <span class="body-medium text-on-surface">${esc(text)}</span>
  </div>`;
}
  ...
  return `<div class="dialog-scrim" data-action="scan.dialogScrim">
    <div class="dialog" role="dialog" aria-modal="true">
      ...
      <div class="dialog-actions">
        ...
        <button class="primary-btn pressable" data-action="scan.dialogSubmit" ${isInviteCode(normalized) && !ui.resolving ? "" : "disabled"} style="width:auto;min-height:44px;border-radius:12px;padding:0 20px">${tx("查询课程", "Find course")}</button>
      </div>
    </div>
  </div>`;
}
...
function joinField({
  ...
  return `<div class="col">
    ...
    ${supporting ? `<div class="field-supporting" data-join-counter="${id}">${esc(supporting)}</div>` : ""}
  </div>`;
}
  ...
  return `<button class="swiss-panel pressable" data-action="join.openStatus" style="text-align:left">
    <div class="row">
      <div class="col grow" style="gap:4px">
        ...
        <span class="body-medium text-muted">${esc(request.courseCode)} / Section ${esc(request.section)} · ${localizedJoinStatus(request.status)}</span>
      </div>
...
export const joinActions = {
  ...
  "joinConfirm.submit": (app) => {
    ...
    setTimeout(() => {
      ...
      app.overlay.joinRequest = request;
      app.saveOverlay();
      app.state.workspace.courseJoinRequest = request;
      ui.submitted = true;
      if (app.state.authenticated) {
        app.ui.joinConfirm = null;
        app.openSub("joinStatus", {});
