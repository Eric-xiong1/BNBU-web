...
export const notificationActions = {
  ...
  "notifications.openNotice": (app, el) => {
    const ui = notificationsState(app);
    const notice = app.visibleNotices().find((n) => n.id === el.dataset.noticeId);
    if (!notice) return;
    if (notice.isUnread) app.markNoticeRead(notice.id);
  ...
  "notifications.markRead": (app, el) => {
    app.markNoticeRead(el.dataset.noticeId);
    app.render();
  },
  "notifications.markAll": (app) => {
    app.markAllNoticesRead();
    app.render();
  },
};
