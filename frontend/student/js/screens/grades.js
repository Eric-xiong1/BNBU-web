...
export function renderGrades(app) {
  ...
  // — Check-in hours card —
  const completed = Math.max(0, progress.course + progress.general);
  const required = Math.max(0, rule.total);
  const remaining = Math.max(0, required - completed);
  const isComplete = required > 0 && completed >= required;
  const ratio = required > 0 ? Math.min(1, completed / required) : 0;
  const hoursCard = `<div class="swiss-panel"><div class="col" style="gap:16px">
    ${cardTitle("check-circle", tx("打卡学时", "Check-in hours"), isComplete ? tx("已完成本学期打卡要求", "Semester check-in requirement complete") : tx(`还需 ${formatHours(remaining)} 小时`, `${formatHours(remaining)} hours remaining`))}
    <div class="row" style="align-items:flex-end">
      <span class="headline-medium text-on-surface">${formatHours(completed)}</span>
      <span class="body-large text-muted" style="padding:0 0 3px 4px">${tx(` / ${formatHours(required)} 小时`, ` / ${formatHours(required)} hours`)}</span>
    </div>
    ${required > 0 ? `<div class="hour-progress" style="height:4px"><div class="fill" style="transform:scaleX(${ratio})"></div></div>` : ""}
    <div class="row">
      <div class="col grow" style="gap:2px">
        <span class="label-medium text-muted">${tx("课程相关", "Course-related")}</span>
        <span class="body-medium text-on-surface" style="font-weight:500">${tx(`${formatHours(progress.course)} / ${formatHours(rule.courseRequired)} 小时`, `${formatHours(progress.course)} / ${formatHours(rule.courseRequired)} hours`)}</span>
      </div>
      <span style="width:16px"></span>
      <div class="col grow" style="gap:2px">
        <span class="label-medium text-muted">${tx("其他运动", "Other exercise")}</span>
        <span class="body-medium text-on-surface" style="font-weight:500">${tx(`${formatHours(progress.general)} / ${formatHours(rule.generalRequired)} 小时`, `${formatHours(progress.general)} / ${formatHours(rule.generalRequired)} hours`)}</span>
      </div>
    </div>
  </div></div>`;
