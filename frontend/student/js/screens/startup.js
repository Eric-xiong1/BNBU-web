...
import { brandLockup, spinner, esc } from "../ui.js";
...
export function renderReadOnlyBanner(app) {
  const message = app.state.systemModeStatus.message;
  return `<div class="banner read-only">
    ${icon("cloud-off", 20)}
    <div class="banner-text">
      <div class="banner-title">${tx("系统只读模式", "System read-only mode")}</div>
      <div class="banner-body">${esc(message) || tx("服务维护中；你仍可查看已有内容，但不能提交或修改。", "Service maintenance is in progress. You can still view existing content, but cannot submit or edit it.")}</div>
    </div>
  </div>`;
}
...
export function renderSyncStatusBanner(app) {
  const message = app.state.lastError
    ?? tx("当前显示缓存数据，内容可能不是最新", "Cached data is shown and may not be up to date.");
