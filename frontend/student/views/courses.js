...
function courseCard(course, records) {
  ...
  return `<article class="card course-card"><div class="card-body">
    ...
    <div class="course-facts"><span>课程相关记录 <b>${courseRecords.length}</b></span></div>
    <button class="button button-tonal button-block" data-route="course/${escapeHtml(course.id)}">查看课程详情</button>
  </div></article>`;
}

export function renderCourses(courses = [], records = []) {
  ...
  return `<section class="page-stack courses-page"><header><span class="eyebrow">COURSES</span><h1 class="page-heading">课程</h1><p class="page-caption">课程代码 + 四位 Section 是唯一课程标识</p></header>
    <section class="page-stack"><div class="section-row"><div><span class="eyebrow">CURRENT</span><h2 class="section-heading">当前学期课程</h2></div><span class="badge">${current.length} 门</span></div>
      <div class="grid grid-2">${current.map((course) => courseCard(course, records)).join("") || '<div class="card"><div class="card-body muted">当前学期暂无课程</div></div>'}</div>
    </section>
    <details class="course-history" ${current.length ? "" : "open"}><summary><span><span class="eyebrow">ARCHIVE</span><strong>历史课程（${archived.length}）</strong></span><b aria-hidden="true">⌄</b></summary><div class="grid grid-2">${archived.map((course) => courseCard(course, records)).join("") || '<div class="card"><div class="card-body muted">暂无历史课程</div></div>'}</div></details>
  </section>`;
}

export function renderCourseDetail(course, records = []) {
  ...
  return `<section class="page-stack course-detail-page"><button class="button button-secondary back-button" data-route="courses">← 返回课程</button>
    ...
    <section class="card"><div class="card-head"><h2 class="card-title">课程相关记录</h2></div><div class="card-body list-stack">${records.map((record) => `<button class="list-row" data-route="record/${record.id}"><span><strong>${record.hours}h · ${formatDate(record.startTime || record.submittedAt)}</strong><small>${escapeHtml(record.description || "")}</small></span><b>→</b></button>`).join("") || '<p class="muted">当前课程还没有课程相关运动记录。</p>'}</div></section>
  </section>`;
}
