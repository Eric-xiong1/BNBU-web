...
export function renderProfile(state) {
  ...
  return `<section class="page-stack profile-page"><header><span class="eyebrow">PROFILE</span><h1 class="page-heading">我的</h1><p class="page-caption">身份、课程联系、组织认证与学生服务</p></header>
    <section class="profile-hero"><div class="avatar">${escapeHtml(student.name?.slice(0,1) || "学")}</div><div><span class="eyebrow">学生身份</span><h2>${escapeHtml(student.name)}</h2><p>${escapeHtml(student.studentNumber || student.id)} · ${escapeHtml(student.college)} · ${escapeHtml(student.gradeLabel)}</p></div></section>
    ...
    <section class="menu-card"><button data-action="open-notifications"><span><b>通知</b><small>${(state.notifications || []).filter((item) => item.isUnread).length} 条未读</small></span><strong>→</strong></button><button data-route="email"><span><b>${state.student?.emailVerified ? "换绑邮箱" : "绑定学校邮箱"}</b><small>${escapeHtml(state.student?.primaryEmailMasked || "完成邮箱验证后可扫码入课")}</small></span><strong>→</strong></button><button data-route="exemptions"><span><b>免测与免打卡</b><small>免测申请、补交材料与审核记录</small></span><strong>→</strong></button><button data-route="help"><span><b>帮助中心</b><small>${state.helpArticles?.length || 0} 篇 Backend 帮助文章</small></span><strong>→</strong></button><button data-route="feedback"><span><b>问题反馈</b><small>提交问题并查看官方回复</small></span><strong>→</strong></button><button data-route="privacy"><span><b>隐私政策</b><small>了解体育数据的采集与使用</small></span><strong>→</strong></button></section>
    <section class="card settings-card"><div class="card-head"><span class="eyebrow">SETTINGS</span><h2 class="card-title">设置</h2></div><div class="card-body page-stack"><label class="field"><span>主题模式</span><select data-setting="themeMode"><option value="light" ${settings.themeMode === "light" ? "selected" : ""}>浅色</option><option value="dark" ${settings.themeMode === "dark" ? "selected" : ""}>深色</option><option value="system" ${settings.themeMode === "system" ? "selected" : ""}>跟随系统</option></select></label><label class="toggle-row"><span><strong>邮件通知</strong><small>由 Backend 保存通知偏好</small></span><input type="checkbox" data-preference="emailEnabled" ${state.preferences?.emailEnabled ? "checked" : ""}></label><label class="toggle-row"><span><strong>推送通知</strong><small>Web Push 注册完成后由 Backend 投递</small></span><input type="checkbox" data-preference="pushEnabled" ${state.preferences?.pushEnabled ? "checked" : ""}></label><label class="toggle-row"><span><strong>减少动态效果</strong><small>关闭非必要页面动画</small></span><input type="checkbox" data-setting="reducedMotion" ${settings.reducedMotion ? "checked" : ""}></label>${state.releasePolicy ? `<div class="notice">Web 版本策略：${escapeHtml(state.releasePolicy.enforcement)} · 最新 ${escapeHtml(state.releasePolicy.latestVersion)}</div>` : ""}${state.mode === "demo" ? '<button class="button button-secondary" data-action="reset-demo">重置演示数据</button>' : ""}</div></section>
