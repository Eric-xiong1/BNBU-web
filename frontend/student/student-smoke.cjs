...
const nginx = fs.readFileSync(path.join(root, "..", "..", "backend", "nginx-snippet.conf"), "utf8");
...
assert.match(app, /\/api\/v1|createStudentApi/);
assert.match(app, /verifyLoginCode/);
assert.doesNotMatch(app, /id="proof-picker"|toggle-password/);
assert.match(app, /saveSessionMedia/);
assert.match(app, /updateBackendPreference/);
assert.match(previewServer, /API_V1_TARGET/);
assert.match(previewServer, /request\.url\.startsWith\("\/api\/v1\/"\)/);
assert.match(nginx, /location \/api\/v1\/[\s\S]*proxy_pass http:\/\/127\.0\.0\.1:3000/);
assert.match(nginx, /location \/api\/[\s\S]*proxy_pass http:\/\/127\.0\.0\.1:3001/);
assert.ok(fs.existsSync(path.join(root, "views", "support.js")));
assert.ok(fs.existsSync(path.join(root, "runtime-config.js")));
